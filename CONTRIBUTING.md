...
Привет! Спасибо, что хочешь помочь с проектом **TintoraAI** — нейросетью для раскраски чёрно-белых фотографий! Этот файл объяснит, как ты можешь внести свой вклад, даже если ты новичок в программировании или GitHub. Мы рады всем, кто хочет улучшить проект!

## Что можно сделать
- Исправить ошибки в коде (например, баги в `src/colorize.py`).
- Добавить новые тесты в `tests/` (например, для полной модели `TintoraAI`).
- Улучшить документацию (этот файл, `README.md`, и другие).
- Добавить новые стили в `src/colorize.py` (например, "cartoon" или "black-and-white").
...
### 4. Согласуй задачу
- Напиши комментарий в выбранном Issue, чтобы сообщить, что ты берёшься за задачу.
...
- Проверь, что можешь запустить тесты:
  ```bash
  cd TintoraAi
  pytest tests/
  ```
- Если есть GPU, убедись, что `torch` поддерживает CUDA:
  ```bash
...
- Если добавляешь функцию, создай тесты для неё в `tests/`. Например, для проверки `src/model/preprocess.py`:
  ```python
  ...
  def test_preprocess():
      ...
      assert tensor.shape == (1, 1, 256, 256), "Wrong tensor shape"
      assert size == (256, 256), "Wrong size"
  ```
