...
def train(config_path="config.yaml"):
    ...
    scaler_G = GradScaler() if torch.cuda.is_available() else None
    scaler_D = GradScaler() if torch.cuda.is_available() else None
    scaler_C = GradScaler() if torch.cuda.is_available() else None
    ...
    for epoch in range(epochs):
        ...
        for i, (bw_images, color_images, labels) in enumerate(tqdm(train_loader, desc=f"Эпоха {epoch+1}/{epochs}")):
            ...
            with autocast(enabled=scaler_D is not None):
            ...
            with autocast(enabled=scaler_G is not None):
            ...
            with autocast(enabled=scaler_C is not None):
                loss_C = loss_class
