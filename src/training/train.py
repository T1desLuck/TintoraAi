    ...
    for epoch in range(epochs):
        ...
        for i, (bw_images, color_images, labels) in enumerate(tqdm(train_loader, desc=f"Эпоха {epoch+1}/{epochs}")):
            ...
            with torch.no_grad():
                ...
                if fake_output.shape[-2:] != color_images.shape[-2:]:
                    ...
                    resized_fake = resized_fake.float()
                    color_images_float = color_images.float()
                    current_ssim = ssim(resized_fake, color_images_float, data_range=1.0, size_average=True).item()
                else:
                    # Преобразуем к типу float32 для совместимости с pytorch_msssim
                    fake_output_float = fake_output.float()
                    color_images_float = color_images.float()
                    current_ssim = ssim(fake_output_float, color_images_float, data_range=1.0, size_average=True).item()
        ...
        with torch.no_grad():
            for bw_images, color_images, labels in val_loader:
                ...
                color_images_float = color_images.float()
                current_ssim = ssim(color_output_float, color_images_float, data_range=1.0, size_average=True).item()
